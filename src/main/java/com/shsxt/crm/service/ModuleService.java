...
public class ModuleService extends BaseService<Module,Integer> {
    ...
    public void saveModule(Module module){
        ...
        AssertUtil.isTrue(null==grade || !(grade==0 || grade==1 || grade ==2),"菜单的层级不合法!!" );
        Module temp=moduleMapper.queryModuleByGradeAndModuleName(module.getGrade(),module.getModuleName());
        AssertUtil.isTrue(null !=temp,"该层级下菜单重复!");
        ...
        AssertUtil.isTrue(null != moduleMapper.queryModuleByOptValue(module.getOptValue()),"权限码以重复!!");
    ...
    public void updateModule(Module module){
        ...
        if(module.getGrade() ==1){
            ...
            if(null != temp ){
                AssertUtil.isTrue(!(temp.getId().equals(module.getId())),"该层级下的url已存在");
            }
        }
    ...
    public void deleteModule(Integer mid){
        ...
        AssertUtil.isTrue(null == mid || null ==temp,"带删除记录不存在!!");
        /**
         * 如果存在子菜单 不许删除
         */

       int count= moduleMapper.countSubModuleByParentId(mid);
       AssertUtil.isTrue(count>0,"存在子级菜单,不支持删除操作");

       //权限表
        count =permissionMapper.countPermissionByModuleId(mid);
        if(count >0){
            AssertUtil.isTrue(permissionMapper.deletePermissionByRoleId(mid)<count,"菜单删除失败!!");
        }
